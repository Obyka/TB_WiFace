...
def read_relationship(id_picture, id_identity):
    ...
    if represent is not None:
        ...
        abort(
            404, "Association {id_picture} - {id_identity} not found".format(id_picture=id_picture, id_identity=id_identity))


@jwt_required
def delete(id_picture, id_identity):
    represent = Represents.query.filter(Represents.fk_identity == id_identity,
                                        Represents.fk_picture == id_picture).one_or_none()
    ...
    if represent is not None:
        ...
        abort(404, "Relationship {id_identity} - {id_picture} not found".format(id_identity=id_identity, id_picture=id_picture))
